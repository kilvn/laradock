...
# 脚本说明：
# 1. 生成自签名证书，并使用 openssl.cnf 配置文件
# 2. 生成 CA 证书，并使用 openssl.cnf 配置文件
# 3. 证书有效期默认为 3650 天
# 4. 证书位数默认为 2048
# 5. 证书 CN 默认为 CN
# 6. 证书扩展 IP 和域名，可根据实际情况添加
# 7. 脚本默认不重新创建 CA 证书，如果需要重新创建，请设置 --ca-cert-recreate=true
# 8. 脚本默认不重新创建 CA 私钥，如果需要重新创建，请设置 --ca-key-recreate=true
# 文档地址：https://docs.rancher.cn/docs/rancher2.5/installation/resources/advanced/self-signed-ssl/_index/
